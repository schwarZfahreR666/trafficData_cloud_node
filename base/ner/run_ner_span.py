...
def main():
    ...
    # 获取对应的processor
    processor = processors[args.task_name]()  # CluenerProcessor()
    label_list = processor.get_labels()   #  ["O", "address", "time","company",'action','government','movie','name','organization','position','scene']
    args.id2label = {i: label for i, label in enumerate(label_list)} # 两个映射字典{0:'0',1:'address'}
    ...
    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_type]   #  (BertConfig, BertSpanForNer, CNerTokenizer)
